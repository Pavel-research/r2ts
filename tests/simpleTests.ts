...
describe("Simple bindings tests", function () {
    ...
    it("buttons", function () {
        var rs = <rp.api10.LibraryBase>rp.loadRAMLSync(path.resolve(__dirname, "../../tests/button.raml"), []);
        var pr = main.process(rs);
    })
    it("temperature", function () {
        var rs = <rp.api10.LibraryBase>rp.loadRAMLSync(path.resolve(__dirname, "../../tests/OICTemperature.raml"), []);
        var pr = main.process(rs);
    })
    it("switch", function () {
        var rs = <rp.api10.LibraryBase>rp.loadRAMLSync(path.resolve(__dirname, "../../tests/binarySwitch.raml"), []);
        var pr = main.process(rs);
    })
})
