...
var typeVisitor_1 = require("../src/typeVisitor");
var main = require("../src/main");
describe("Simple bindings tests", function () {
    it("schema with reference, example is valid", function () {
        var rs = rp.loadRAMLSync(path.resolve(__dirname, "../../tests/types.raml"), []);
        var nmm = new typeVisitor_1.JavaScriptMetaEmmitter();
        nmm.defaultPath = rs.highLevel().lowLevel().unit().absolutePath();
        rs.types().forEach(function (x) {
            if (x.name() == "HasId") {
                var rs = nmm.visit(x);
    ...
    it("core types should not be prefixed", function () {
        var rs = rp.loadRAMLSync(path.resolve(__dirname, "../../tests/tp2.raml"), []);
    ...
    it("library types should be prefixed", function () {
        var rs = rp.loadRAMLSync(path.resolve(__dirname, "../../tests/tp3.raml"), []);
        var pr = main.process(rs);
        assert.deepEqual(pr.types, {
            "HasAnimals": {
                "id": "HasAnimals",
    ...
    it("library types should be prefixed", function () {
        var rs = rp.loadRAMLSync(path.resolve(__dirname, "../../tests/tp4.raml"), []);
        var pr = main.process(rs);
        assert.deepEqual(pr.types, {
            "Type": {
                "id": "Type",
    ...
    it("github0", function () {
        var rs = rp.loadRAMLSync(path.resolve(__dirname, "../../tests/github0.raml"), []);
        var pr = main.process(rs);
    });
    it("github1", function () {
        var rs = rp.loadRAMLSync(path.resolve(__dirname, "../../tests/github1.raml"), []);
        var pr = main.process(rs);
